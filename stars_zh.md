...
> `#C/C++` `#互动桌宠` `#桌面应用` `#跨平台` `#bongo-cat` `#bongocat` `#c` `#cross-platform` `#desktop-app` `#desktop-pet` `#linux` `#macos` `#pet` `#pet-project` `#windows` 
> BongoCat是一款跨平台互动桌宠应用，支持Windows、macOS和Linux。猫咪会在桌面上自由行走、互动，为桌面增添趣味与陪伴感。项目正从Tauri迁移至C/C++、SDL3和OpenGL的原生实现，轻量高效，适合个性化桌面装饰与解压场景。
