...
> `#BitTorrent` `#下载管理器` `#桌面应用` `#aria2` `#bittorrent` `#bt` `#download` `#download-manager` `#electron` `#linux` `#mac` `#macos` `#magnet` `#motrix` `#torrent` `#windows` 
...
> `#macOS工具` `#磁盘清理` `#系统优化` `#analyzer` `#appcleaner` `#clean` `#cleaner` `#cleanmymac` `#command-line` `#daisydisk` `#istat` `#mac` `#macos` `#macos-app` `#native` `#optimize` `#sensei` `#shell` `#swift` `#swiftui` `#uninstall` 
...
> `#导航站` `#网站管理` `#自动化爬虫` `#ai` `#aigotools` `#directory` `#nestjs` `#nextjs` `#openai` `#tools` 
> AigoTools 是一款导航站建站工具，支持快速创建和管理网站目录，内置自动爬虫抓取站点信息（基于 Playwright、Jina、OpenAI），提供用户管理、国际化、SEO 优化及多平台图片存储（MinIO、AWS S3、腾讯云 COS）。用户可一键部署个性化导航网站，适合内容聚合与分类管理场景。
