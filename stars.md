...
> [!info]
> ⭐ 253 · Python · 2026-01-21  
> [GitHub](https://github.com/op7418/Document-illustrator-skill)  
> `#AI配图` `#文档可视化` `#社交媒体` `#Claude Skill` `#多风格生成` `#智能归纳` `#Python工具` `#Gemini API` >
> 基于AI的文档配图生成工具，支持多格式文档智能解析，内置渐变玻璃、票据、矢量插画三种风格，可生成16:9和3:4比例图片，适用于小红书、推特等社交媒体内容创作。
...
> `#n8n` `#自动化工作流` `#开源` `#无代码` `#Next.js` `#Supabase` `#模板库` `#自托管` >
> 提供8000+个已验证的n8n自动化工作流，涵盖营销、DevOps、销售与AI场景，支持一键导入。包含可自托管的Next.js前端，支持按角色或集成筛选搜索，数据以JSON和SQL格式开放，助力快速实现无代码自动化。
...
> <sub>Complete Claude Code configuration collection - agents, skills, hooks, commands, rules, MCPs. Battle-tested configs from an Anthropic hackathon winner.</sub>
...
> `#AI助手框架` `#国产大模型` `#多平台机器人` `#Function Calling` `#轻量级架构` `#飞书钉钉集成` `#会话管理` `#插件系统` `#agent` `#claude` `#claude-code` `#clawdbot` `#deepseek` `#dingding` `#feishu` `#minimax` `#moltbot` `#openclaw` `#qq` `#qqbot` >
> OpenMozi 是一个轻量级、专注国产生态的 AI 助手框架，支持 DeepSeek、豆包、Qwen 等国产大模型及飞书、钉钉、QQ、企业微信等通讯平台。提供 Function Calling、25+ 内置工具、技能系统、记忆与定时任务等核心能力，代码简洁（约1.6万行），适合企业内部机器人开发与 Agent 原理学习。
...
> `#虚拟麦克风` `#WebRTC` `#音频流` `#局域网传输` `#跨平台` `#Node.js` `#虚拟声卡` `#低延迟` >
> ToMic 是一款基于 Web 技术的局域网虚拟麦克风工具，支持通过手机浏览器将音频实时传输至电脑，并利用虚拟声卡（如 BlackHole 或 VB-CABLE）注入系统输入。适用于 Zoom、Teams 等会议场景，无需安装手机 App，具备低延迟、HTTPS 安全连接及跨平台原生监听支持。
...
> `#AI 内容生成` `#短视频制作` `#漫画创作` `#角色管理` `#分镜生成` `#Gemini API` `#VEO API` `#Next.js` >
> 基于 Gemini（Nano Banano 和 Veo）的短视频与漫画创作工具，支持分镜生成、角色一致性管理、剧本导入及画布视图，提供端到端 AI 内容创作流程。
...
> `#Telegram Bot` `#Claude Code` `#远程控制` `#长期记忆` `#SQLite` `#会话管理` `#代码高亮` `#隐私安全` >
> MateCode 是一个通过 Telegram 远程控制 Claude Code 的桥接工具，支持长期记忆、会话管理、代码高亮和本地 SQLite 记忆系统，采用纯轮询模式保障隐私安全，无需公网暴露。
...
> [!info]
> ⭐ 1,274 · TypeScript · 2026-02-25  
> [GitHub](https://github.com/workany-ai/workany) · [Website](https://workany.ai)  
> `#AI Agent` `#桌面应用` `#自然语言处理` `#代码生成` `#沙箱执行` `#多模型支持` `#MCP 协议` `#Tauri` `#ai-assistant` `#desktop-agent` `#general-agent` `#open-cowork` >
> WorkAny 是一款桌面 AI 智能体应用，支持通过自然语言执行任务，具备实时代码生成、工具调用、工作区管理及沙箱安全执行能力，集成多模型提供商与 MCP 协议，适用于文件整理、网页/文档/表格/幻灯片生成等场景。
...
> `#知识图谱` `#AI 发散思维` `#Vue 3` `#TypeScript` `#本地优先` `#交互式可视化` `#Markdown 渲染` `#配图生成` `#ai` `#flow` `#gpt` `#thinkflow` `#web` >
> ThinkFlow AI 是一个轻量级“从想法到结构化图谱”的工作台，用户输入核心想法后，系统自动扩展为可交互浏览的知识图谱。支持节点追问、深度解析与配图生成，具备智能发散、深度挖掘和极致交互体验，采用 Vue 3 + Vite + TypeScript 构建，本地优先架构保障数据隐私。
...
> [!info]
> ⭐ 224 · Python · 2026-02-11  
> [GitHub](https://github.com/lingfengQAQ/webnovel-writer)  
> `#AI写作` `#网文创作` `#Claude Code` `#防幻觉机制` `#长篇连载` `#质量审查` `#RAG检索` `#叙事节奏` >
> 基于Claude Code的AI网文创作辅助系统，支持200万字级长篇连载管理，通过双Agent架构、防幻觉三定律、六维质量审查和RAG检索，解决AI写作中的遗忘与逻辑矛盾问题，内置创意约束、追读力机制与三线叙事节奏控制，保障内容一致性与读者吸引力。

> <sub>AI驱动的长篇网文辅助创作工具 - 基于Claude Code，支持200万字连载管理，内置防幻觉机制和5维质量审查</sub>
...
> `#Flask` `#PyAutoGUI` `#剪贴板同步` `#移动端输入` `#实时传输` `#Windows自动化` `#跨设备输入` `#Python` >
> EasyType 是一款基于 Flask 的跨平台工具，支持通过手机浏览器实时将输入内容推送到 Windows 电脑，提供直接键盘输入或剪贴板复制两种模式，适用于远程输入、演示辅助等场景，具备低延迟和移动端优化界面。
...
> [!info]
> ⭐ 1,222 · Rust · 2026-02-25  
> [GitHub](https://github.com/aiclientproxy/proxycast)  
> `#AI 创作助手` `#内容生成` `#多模态输出` `#创作工作流` `#自媒体工具` `#小说写作` `#短视频脚本` `#知识管理` `#agent` `#agentic-ai` `#ai` `#antigravity` `#gemini` `#kiro` >
> ProxyCast 是一款面向普通创作者的 AI Agent 平台，集成灵感激发、内容生成、图文创作、迭代修改与成果沉淀于一体，支持社媒、短视频、小说、歌词等多元创作场景，实现从创意到发布的端到端高效工作流。
...
> [!info]
> ⭐ 399 · JavaScript · 2026-02-24  
> [GitHub](https://github.com/doocs/cose)  
> `#Markdown` `#浏览器扩展` `#内容同步` `#多平台发布` `#微信公众号` `#开发者工具` `#开源` `#doocs` `#cose` `#doocs` `#extension` `#extension-chrome` `#md` >
> COSE 是一款与 doocs/md Markdown 编辑器配合使用的浏览器扩展，支持一键将文章同步至微信公众号、知乎、CSDN、掘金等 30+ 内容平台，具备本地运行、样式保留、自动分组等特性，提升多平台内容分发效率。
...
> [!info]
> ⭐ 8,846 · JavaScript · 2026-02-12  
> [GitHub](https://github.com/fishjar/kiss-translator) · [Website](https://github.com/fishjar/kiss-translator)  
> `#双语翻译` `#浏览器扩展` `#油猴脚本` `#AI翻译` `#YouTube字幕` `#自定义规则` `#开源` `#多语言支持` `#extension` `#translate` `#translation` `#translator` `#userscript` `#userscripts` >
> KISS Translator 是一款简约开源的双语对照翻译工具，支持浏览器扩展与油猴脚本。提供网页翻译、划词翻译、输入框翻译、YouTube字幕翻译等功能，兼容Google、OpenAI、DeepL等主流翻译服务，支持自定义规则、AI上下文记忆及富文本保留，适配Chrome、Firefox、Safari等主流浏览器。
...
> `#微信推送` `#Go语言` `#HTTP API` `#Docker部署` `#模板消息` `#Webhook` `#免费服务` `#自动化通知` >
> 基于 Go 开发的免费微信消息推送服务，支持通过 HTTP GET/POST 接口向微信用户发送模板消息，具备原生弹窗提醒、多用户支持、Docker 一键部署（镜像仅2MB）、每日10万次额度及自定义跳转详情页等功能，适用于个人通知与自动化场景。
...
> [!info]
> ⭐ 1,618 · TypeScript · 2026-01-14  
> [GitHub](https://github.com/RanFeng/clipsketch-ai) · [Website](https://clipsketch-ai.vercel.app)  
> `#AI 内容生成` `#视频剪辑` `#手绘故事板` `#Gemini API` `#社交媒体文案` `#React` `#TypeScript` `#多平台适配` >
> ClipSketch AI 是一款 AI 驱动的视频创作工具，支持解析 Bilibili 和小红书视频链接，实现帧级标记精彩瞬间，并借助 Google Gemini 多模态模型将标记帧转化为手绘风格故事板，同时自动生成适配社交媒体的爆款文案与封面，助力内容创作者高效产出。
...
> [!info]
> ⭐ 197 · TypeScript · 2025-11-27  
> [GitHub](https://github.com/wordflowlab/article-writer)  
> `#AI写作助手` `#微信公众号` `#多模式创作` `#真实内容生成` `#斜杠命令` `#文档爬虫` `#素材库管理` `#AI味检测` `#ai-article-writer` `#article` `#spec-kit` >
> AI驱动的智能写作工具，专为微信公众号/自媒体创作设计。支持Claude、Cursor、Gemini等13个AI平台斜杠命令，提供教练/快速/混合/框架约束四种写作模式，强调真实经历融入与低AI味输出，集成素材库、文档爬虫、三遍审校及微信格式化功能。
...
> [!info]
> ⭐ 638 · Shell · 2025-10-26  
> [GitHub](https://github.com/wordflowlab/novel-writer)  
> `#AI 小说创作` `#规格驱动开发` `#斜杠命令` `#中文写作工具` `#多平台支持` `#情节追踪` `#插件系统` `#七步方法论` `#engineering` `#novels` `#sdd` `#spec` `#spec-driven` >
> Novel Writer 是一款基于规格驱动开发（SDD）的 AI 驱动中文小说创作工具，支持在 Claude、Cursor、Gemini 等 13 个 AI 助手中通过斜杠命令调用。提供七步系统化创作流程、情节追踪、角色一致性验证及插件扩展功能，专为高质量中文小说创作优化，跨平台兼容。
...
> [!info]
> ⭐ 4,730 · N/A · 2026-01-31  
> [GitHub](https://github.com/666OS/ClashMac) · [Website](https://clashmac.app)  
> `#macOS` `#代理工具` `#SwiftUI` `#流量可视化` `#TUN模式` `#隐私保护` `#订阅管理` `#原生应用` `#clash` `#clash-meta` `#macos` `#mihomo` `#network` `#proxy` `#swiftui` >
> ClashMac 是一款专为 macOS 打造的原生代理工具，基于 SwiftUI 开发，提供系统级代理与 TUN 增强双模式。核心亮点包括全球路由地图可视化、实时连接拓扑图、4D 流量统计及隐私保护功能，支持订阅管理、一键规则配置和菜单栏快捷操作，兼顾高性能与用户体验。
...
> `#下载工具` `#CLI` `#Docker` `#音视频下载` `#播客` `#WebDAV` `#多语言` `#跨平台` >
> vget 是一款多功能下载工具，支持音视频、播客、PDF 等多种格式，兼容 Twitter、小红书、小宇宙等平台。提供 CLI 和 Docker 部署方式，支持 WebDAV 远程管理、多语言配置及自更新功能，适合跨平台高效下载需求。
...
> [!info]
> ⭐ 75 · TypeScript · 2025-09-17  
> [GitHub](https://github.com/millylee/auo)  
> `#Claude Code` `#多配置切换` `#环境变量管理` `#API 代理` `#命令行工具` `#自动升级` `#向后兼容` `#Node.js` `#anyrouter` `#claude` `#claude-code` `#cli` `#glm` `#kimi` >
> auo 是一个增强 Claude Code 使用体验的工具，支持多配置切换与环境变量管理，适用于多账号或代理中转场景。主要特性包括配置自动升级、向后兼容、交互式配置管理及无缝集成 Claude Code 命令。
...
> `#微信公众号` `#Markdown转换` `#文章抓取` `#批量下载` `#图片防盗链` `#命令行工具` `#Python` `#内容提取` >
> WeSpy 是一款专为微信公众号设计的 Python 工具，支持从公众号文章或专辑中提取内容并转换为 Markdown 格式，具备图片防盗链处理、多格式输出（HTML/JSON/Markdown）、批量下载及命令行交互等特性，适用于内容归档与研究场景。
