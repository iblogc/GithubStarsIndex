...
> OpenCLI 是一个强大的 AI 原生浏览器自动化工具，可将任意网站或 Electron 应用转换为命令行接口。它支持会话复用、AI 驱动的发现与合成，提供零风险的数据提取和跨平台自动化能力，适用于 Bilibili、知乎、小红书等主流平台。
...
> `#AI智能体` `#多平台交互` `#浏览器自动化` 
> opencli-skill 是一个 Claude Code 技能，通过复用 Chrome 登录态直接操控 B站、知乎、Twitter/X、YouTube 等 16 大平台，支持内容浏览、搜索与发布，无需 API Key，实现自然语言驱动的跨平台自动化交互。
...
> `#AI智能体` `#MCP工具调用` `#多模型支持` `#桌面应用` `#agent` `#agent-skills` `#agentic` `#agentic-ai` `#ai` `#ai-assistant` `#ai-chat` `#chatbot` `#chatgpt` `#claude` `#cross-platform` `#deepseek` `#electron` `#gemini` `#llm-client` `#mcp` `#mcp-client` `#openai-client` `#tool-calling` 
