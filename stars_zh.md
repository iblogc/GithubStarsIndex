...
> `#微信工具` `#数据导出` `#桌面应用` `#隐私保护` `#ai-memory` `#knowledge-management` `#llm-context` `#local-first` `#pdf-export` `#personal-knowledge-base` `#pyside6` `#wechat` `#windows` 
> 微信 AI 记忆库是一款本地优先的桌面工具，直接从 Windows 微信 4.x 读取聊天记录，支持按会话、日期和关键词筛选，可导出 PDF、Markdown 与结构化 JSON。全程仅操作临时数据库副本，图片恢复、语音转写（faster-whisper）均在本机完成，无任何数据上传。
...
> `#开源工具` `#显示器管理` `#桌面应用` `#4k` `#apple-silicon` `#brightness` `#brightness-control` `#ddc` `#display` `#display-manager` `#external-monitor` `#hidpi` `#mac` `#macos` `#macos-app` `#menu-bar` `#menubar` `#monitor` `#resolution` `#retina` `#scaling` `#swift` `#virtual-display` 
