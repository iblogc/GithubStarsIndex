...
> `#AI智能体` `#DeepSeek模型` `#终端开发` `#自动化编程` `#agent-orchestration` `#ai-agent` `#cli` `#coding-agent` `#local-first` `#mcp` `#multi-agent` `#multi-model` `#open-source` `#rust` `#tui` 
...
> `#React` `#UI 动效组件` `#前端开发` `#beam` `#design` `#effect` `#glow` `#motion` `#product` 
> 该仓库提供两个 React UI 动效组件库：border-beam 实现流光边框动画，liquid-gooey 提供液态变形与流动特效。适用于现代 Web 应用视觉增强，支持源码热更新、独立 npm 发布与 CI 构建部署。
