...
> `#AI智能体` `#多智能体协作` `#本地部署` `#agent` `#agentscope` `#agentscope-runtime` `#llm-tools` `#llms` `#skills` 
...
> `#AI智能体` `#会话管理` `#多平台集成` `#agent` `#ai-agent` `#chat-ui` `#dashboard` `#hermes` `#hermes-agent` `#hermes-web-ui` `#llm` `#multi-model` `#multi-platform` `#self-hosted` `#typescript` `#vue3` `#web-ui` 
> Hermes Web UI 是一个功能完整的 Hermes Agent 管理仪表盘，支持多平台 AI 聊天会话管理、实时消息流、定时任务调度、使用量分析与成本追踪，以及 Telegram、Discord、Slack 等八种通信渠道的统一配置，提供本地 SQLite 会话存储与全局搜索能力。
...
> `#AI智能体` `#Markdown排版` `#公众号自动化` `#agent-cli` `#ai-agent` `#ai-writing` `#claude-code` `#claude-skills` `#cli` `#go` `#llm` `#markdown` `#markdown-converter` `#markdown-formatter` `#markdown-to-html` `#markdown-to-wechat` `#mcp-server` `#md2wechat` `#wechat` `#wechat-article` `#wechat-mp` `#wechat-public-account` `#weixin` 
...
> `#AI智能体` `#IDE工具` `#多智能体协作` `#ade` `#claude-code` `#cli` `#codex` `#cursor-agent` `#ghostty` `#ide` `#mobile-app` `#opencode` `#orchestration` `#parallel-agents` `#pi` `#terminal` `#worktrees` 
...
> `#AI智能体` `#企业AI` `#自动化工具` `#语义搜索` `#agents` `#ai` `#analytics` `#artificial-inteligence` `#bigquery` `#business-intelligence` `#databases` `#hacktoberfest` `#llms` `#mcp` `#mssql` `#mysql` `#postgresql` `#rag` 
> Minds Platform 是一个开源的人工智能基础平台，旨在为开发者、企业和个人提供可完全控制、扩展和随处部署的生产级AI系统。其核心包括 Anton 自动化代理和 Query Engine 语义搜索引擎，支持自动化任务执行、跨数据源智能检索与分析，适用于报表生成、企业知识库搜索、AI运营助手等场景，支持云、VPC、本地及混合部署。
...
> `#AI智能体` `#RAG 检索增强` `#无代码工具` `#桌面应用` `#agent-harness` `#agentic-ai` `#ai-agents` `#hermes-agent` `#image-classification` `#llm` `#local-ai` `#local-first` `#localai` `#multimodal` `#no-code` `#open-claw` `#rag` `#vector-database` 
