...
> `#AI智能体` `#Android应用` `#聊天助手` 
> Jev 聊天助手是一款 Android 对话副驾应用，通过无障碍服务读取微信/QQ/X/飞书等聊天内容，结合 TypeSafe Jev 判断对方意图与危险等级，并用 LLM 生成 3 条候选回复，一键填入输入框。不自动发送，隐私本地存储，支持多平台适配扩展。
...
> `#AI大模型` `#角色模拟` `#语言特征` `#agent-skill` `#ai-writing` `#character-modeling` `#ghostwriting` `#persona` `#prompt-engineering` `#python` `#style-transfer` 
